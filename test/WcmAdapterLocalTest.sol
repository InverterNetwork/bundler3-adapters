...
contract WcmAdapterLocalTest is Test {
    ...
    function setUp() public {
        ...
        wcmAdapter = new WcmAdapter(
            address(bundler3), address(morpho), address(wcmRouter), address(loanToken), address(collateralToken)
        );
    ...
    function testConstructor() public {
        ...
        new WcmAdapter(address(0), rdmAddress, rdmAddress, rdmAddress, rdmAddress);

        vm.expectRevert(ErrorsLib.ZeroAddress.selector);
        new WcmAdapter(rdmAddress, address(0), rdmAddress, rdmAddress, rdmAddress);

        vm.expectRevert(ErrorsLib.ZeroAddress.selector);
        new WcmAdapter(rdmAddress, rdmAddress, address(0), rdmAddress, rdmAddress);

        vm.expectRevert(ErrorsLib.ZeroAddress.selector);
        new WcmAdapter(rdmAddress, rdmAddress, rdmAddress, address(0), rdmAddress);

        vm.expectRevert(ErrorsLib.ZeroAddress.selector);
        new WcmAdapter(rdmAddress, rdmAddress, rdmAddress, rdmAddress, address(0));

        vm.expectRevert(ErrorsLib.InvalidWcmPair.selector);
        new WcmAdapter(rdmAddress, rdmAddress, rdmAddress, rdmAddress, rdmAddress);
    }
    ...
    function testBuyForwardsDeltaAndLeavesSourceSkimmable() public {
        uint256 amount = 10e18;
        uint256 extra = 3e18;
    ...
    function testBuyMorphoDebtInvalidMarket() public {
        ...
        vm.expectRevert(ErrorsLib.InvalidWcmPair.selector);
        vm.prank(address(bundler3));
        wcmAdapter.buyMorphoDebt(
            address(collateralToken), invalidMarketParams, 1, address(this), RECEIVER, block.timestamp
        );
    }
